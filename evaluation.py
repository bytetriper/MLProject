...
class QuickTrain():
    def __init__(self,model:nn.Module,target_model=None,load:bool=True) -> None:
        ...
        self.optimizer=torch.optim.Adam(self.model.net.parameters(),lr=2e-3)
    ...
    def train(self,train_loader:DataLoader,valid_loader:DataLoader,epoch:int,saved:bool):
        for i in range(epoch):
            ...
            for batch in train_loader:
                ...
                tbar.set_postfix(loss=avg_loss,lr=self.optimizer.param_groups[0]['lr']) 
            self.scheduler.step()
...
def quick_train(batch_size:int=256,epoch:int=10):
    dataset=datasets.load_from_disk(Params["transfer_dataset_path"],keep_in_memory=True)
    ...
    train_loader=DataLoader(train_set,batch_size=batch_size,shuffle=True,num_workers=15,pin_memory=True,pin_memory_device="cuda:0")
    valid_loader=DataLoader(valid_set,batch_size=batch_size,shuffle=False,num_workers=15)
    trainer=QuickTrain(Test_Resnet(),load=True)
    trainer.train(train_loader,valid_loader,epoch,True)
    trainer.eval(valid_loader)
def test_on_classifier(target_model:nn.Module=None,batch_size:int=256):
    print("Testing on classifier on transfer dataset:tiny-imagenet with batch_size=",batch_size)
    dataset=datasets.load_from_disk(Params["transfer_dataset_path"],keep_in_memory=True)['train']
    dataset.set_format(type='torch',columns=['image','label'])
    loader=DataLoader(dataset,batch_size=batch_size,shuffle=False,num_workers=15)
    model=Test_Resnet()
